...
int main(void) {
    ...
    assert(cfg3.relaySignalEnabled == 1);
    ...
    assert(strcmp(cfg4.relayHost, "") == 0); // default is blank, relay_host absent
    assert(cfg4.relayPort == 24689); // default
    assert(cfg4.relaySignalEnabled == 0); // default
    printf("Partial ini falls back to defaults correctly: PASSED\n");

    // relay fields are deliberately NOT in kMenuItems (no settings-UI
    // row for them, same as v17) -- confirm that's still true, since a
    // future menu-schema change accidentally re-adding one of these
    // three would be easy to miss otherwise.
    for (int i = 0; i < kMenuItemCount; i++) {
        assert(strcmp(kMenuItems[i].key, "relay_host") != 0);
        assert(strcmp(kMenuItems[i].key, "relay_port") != 0);
        assert(strcmp(kMenuItems[i].key, "relay_signal_enabled") != 0);
    }
    printf("Relay fields confirmed absent from kMenuItems: PASSED\n");
